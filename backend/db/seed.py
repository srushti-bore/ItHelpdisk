...
from backend.models.ai_artifacts import AITriageResult, CaseRiskAssessment
...
from backend.models.message import CommunicationDraft, Message
...
async def seed_database():
    print("🌱 [Seeder] Starting IT Helpdesk Demo Data Seeder...")
    ...
    async with AsyncSessionLocal() as db:
        ...
        users_data = [
            ...
            {
                "email": "manager@ithelpdesk.local",
            ...
            {
                "email": "operator.pune@ithelpdesk.local",
            ...
            {
                "email": "operator.blr@ithelpdesk.local",
            ...
            {
                "email": "knowledge.owner@ithelpdesk.local",
            ...
            {
                "email": "requester@ithelpdesk.local",
        ...
        cases_data = [
            {
                "ref": "INC-20260916-0001",
                "type": CaseType.INCIDENT,
                "category": CaseCategory.NETWORK,
                "title": "Production VPN Gateway Unreachable - Remote Engineers Impacted",
                "description": "The primary GlobalProtect VPN portal (vpn.corp.company.com) is timing out with 504 Gateway errors. Over 50 remote developers are unable to access production clusters.",
                "priority": Priority.P1_CRITICAL,
                "severity": Severity.SEV1_CRITICAL,
                "status": CaseStatus.ASSIGNED,
                "site": "Pune",
                "assigned_to": users_map["operator.blr@ithelpdesk.local"],
                "assigned_team": teams_map["Network Operations (NOC)"],
                "requester": users_map["requester@ithelpdesk.local"],
                "created_at": now - timedelta(minutes=10),
                "risk_score": 85,
                "risk_level": RiskLevel.HIGH,
            },
            {
                "ref": "INC-20260916-0002",
                "type": CaseType.INCIDENT,
                "category": CaseCategory.INFRASTRUCTURE,
                ...
                "severity": Severity.SEV2_MAJOR,
                "status": CaseStatus.IN_PROGRESS,
                "site": "Bengaluru",
                "assigned_to": users_map["operator.pune@ithelpdesk.local"],
            ...
            {
                "ref": "REQ-20260916-0003",
                "type": CaseType.SERVICE_REQUEST,
                "category": CaseCategory.ACCESS_IDENTITY,
                ...
                "severity": Severity.SEV3_MODERATE,
                "status": CaseStatus.WAITING_ON_REQUESTER,
                "site": "Pune",
                "assigned_to": users_map["operator.pune@ithelpdesk.local"],
            ...
            {
                "ref": "INC-20260916-0004",
                "type": CaseType.INCIDENT,
                "category": CaseCategory.HARDWARE,
        ...
        for c_data in cases_data:
            ...
            if not case:
                case = Case(
                    ...
                    assigned_to_id=c_data["assigned_to"].id if c_data.get("assigned_to") else None,
                    assigned_team_id=c_data["assigned_team"].id if c_data.get("assigned_team") else None,
                ...
                sla = SLA(
                    ...
                    first_responded_at=case.created_at + timedelta(minutes=5) if case.status != CaseStatus.INGESTED else None,
                ...
                triage = AITriageResult(
                    case_id=case.id,
                    suggested_category=case.category.value,
                    suggested_severity=case.severity.value,
                    ...
                    suggested_team_id=case.assigned_team_id,
                ...
                if c_data.get("is_breached"):
                    esc = EscalationEvent(
                        case_id=case.id,
                        escalation_level=EscalationLevel.LEVEL_2_TEAM_LEAD,
                        status=EscalationStatus.ACTIVE,
                        reason="SLA resolution breached 8-hour target without resolution update.",
        ...
        for a_data in articles_data:
            ...
            if not article:
                article = KnowledgeArticle(
                    ...
                    body=a_data["body"],
                    state=a_data["state"],
                    author_id=users_map["knowledge.owner@ithelpdesk.local"].id,
                )
                db.add(article)

        await db.commit()

    print("✅ [Seeder] IT Helpdesk Demo Data Successfully Seeded!")
    print("\n--- Demo Login Credentials ---")
    print("👑 Admin:            admin@ithelpdesk.local          / AdminPassword123!")
    print("👔 Manager:          manager@ithelpdesk.local        / ManagerPassword123!")
    print("🛠️  Operator (Pune):  operator.pune@ithelpdesk.local  / OperatorPassword123!")
    print("🛠️  Operator (BLR):   operator.blr@ithelpdesk.local   / OperatorPassword123!")
    print("📚 Knowledge Owner:  knowledge.owner@ithelpdesk.local / KnowledgePassword123!")
    print("👤 Requester:        requester@ithelpdesk.local      / RequesterPassword123!")
    print("------------------------------\n")
